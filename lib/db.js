...
const DATA_DIR = resolveAppPaths(process.env).dataDir;
...
const stmts = {
  ...
  qualityBySession: db.prepare(
    `SELECT faithfulness, relevancy, confidence, ragResultCount, createdAt
     FROM quality_scores WHERE sessionId = ?
     ORDER BY createdAt DESC LIMIT ?`
  ),
...
function getRecentQualityScores(sessionId, limit = 5) {
  try {
    return stmts.qualityBySession.all(sessionId, limit);
  } catch (err) {
    console.error("[db] getRecentQualityScores error:", err.message);
    return [];
  }
}
