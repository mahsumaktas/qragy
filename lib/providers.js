...
function buildGeminiGenerationConfig(maxOutputTokens, thinkingBudget, withThinking, temperatureOverride) {
  const config = { temperature: temperatureOverride !== undefined ? temperatureOverride : 0.2, maxOutputTokens };
...
async function callGeminiProvider(messages, systemPrompt, maxOutputTokens, cfg) {
  ...
  // enableThinking: "true" = always send, "false" = never send, "auto" = try, auto-disable on error
  ...
  const body = {
    ...
    contents: messages,
    generationConfig: buildGeminiGenerationConfig(maxOutputTokens, cfg.thinkingBudget, shouldTryThinking, cfg.temperature),
  };

  try {
    return await executeGeminiRequest(endpoint, body, cfg);
  } catch (error) {
    // Auto-fallback: if thinking not supported, retry without thinking
    if (enableThinking === "auto" && shouldTryThinking && isThinkingNotSupportedError(error)) {
      console.warn("[Gemini] Model does not support thinking, auto fallback (thinking disabled)");
      body.generationConfig = buildGeminiGenerationConfig(maxOutputTokens, cfg.thinkingBudget, false, cfg.temperature);
      return await executeGeminiRequest(endpoint, body, cfg);
    }
    throw error;
  }
...
const FALLBACK_STATUS_CODES = new Set([404, 429, 500, 503, 504]);
...
async function callLLMWithFallback(messages, systemPrompt, maxOutputTokens, options) {
  ...
  for (let i = 0; i < models.length; i++) {
    const maxAttempts = i === 0 ? 2 : 1; // Primary model: 1 retry on timeout
    for (let attempt = 0; attempt < maxAttempts; attempt++) {
      try {
        ...
        // Primary model timeout: retry once before fallback
        if (status === 504 && attempt < maxAttempts - 1) {
          console.warn(`[Retry] ${models[i]} timeout, attempt ${attempt + 2}/${maxAttempts}`);
          continue;
        }
        const canFallback = FALLBACK_STATUS_CODES.has(status) && i < models.length - 1;
        if (canFallback) {
          console.warn(`[Fallback] ${models[i]} failed (${status}), next: ${models[i + 1]}`);
          break; // Break inner retry loop, continue to next model
        }
        // All models exhausted — user-friendly error
        if (i > 0) {
          console.error(`[Fallback] All models failed (${models.join(" → ")})`);
          const friendly = new Error("All AI models are currently busy. Please try again in a few minutes.");
          friendly.status = 503;
          throw friendly;
        }
        throw error;
