...
// Models that support thinking — add new ones as Google releases them
...
function modelSupportsThinking(modelName) {
  ...
  for (const m of THINKING_CAPABLE_MODELS) {
    if (lower.includes(m)) return true;
  }
  // Heuristic: 2.5+ models generally support thinking
  return /gemini-2\.[5-9]/.test(lower);
}

function buildGeminiGenerationConfig(maxOutputTokens, thinkingBudget, modelName) {
  const config = { temperature: 0.2, maxOutputTokens };
  if (thinkingBudget > 0 && modelSupportsThinking(modelName)) {
  ...
  try {
    response = await fetch(endpoint, {
      ...
      body: JSON.stringify({
        ...
        contents: messages,
        generationConfig: buildGeminiGenerationConfig(maxOutputTokens, cfg.thinkingBudget, cfg.model),
      }),
    });
  } catch (error) {
