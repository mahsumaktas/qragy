<script>
  ...
  onMount(async () => {
    try {
      ...
      showToast("Failed to load bot settings: " + e.message, "error");
    } finally {
      loading = false;
    }
  });
  ...
  async function saveChatFlow() {
    try {
      ...
      showToast("Chat flow saved", "success");
    } catch (e) {
      showToast("Error: " + e.message, "error");
    }
  }
...
<div class="page-header">
  <div>
    <h1>Bot Settings</h1>
    <p>Bot behavior and configuration</p>
  </div>
...
{:else}
  <ExpandCard title="Identity (soul.md)">
    <textarea class="mono-editor" rows="12" bind:value={soulMd}></textarea>
    <div class="card-actions"><Button onclick={() => saveFile("soul.md", soulMd)} variant="primary" size="sm">Save</Button></div>
  </ExpandCard>

  <ExpandCard title="Persona (persona.md)">
    <textarea class="mono-editor" rows="12" bind:value={personaMd}></textarea>
    <div class="card-actions"><Button onclick={() => saveFile("persona.md", personaMd)} variant="primary" size="sm">Save</Button></div>
  </ExpandCard>

  <ExpandCard title="Domain (domain.md)">
    <textarea class="mono-editor" rows="10" bind:value={domainMd}></textarea>
    <div class="card-actions"><Button onclick={() => saveFile("domain.md", domainMd)} variant="primary" size="sm">Save</Button></div>
  </ExpandCard>

  <ExpandCard title="Skills (skills.md)">
    <textarea class="mono-editor" rows="10" bind:value={skillsMd}></textarea>
    <div class="card-actions"><Button onclick={() => saveFile("skills.md", skillsMd)} variant="primary" size="sm">Save</Button></div>
  </ExpandCard>

  <ExpandCard title="Hard Bans (hard-bans.md)">
    <textarea class="mono-editor" rows="8" bind:value={hardBansMd}></textarea>
    <div class="card-actions"><Button onclick={() => saveFile("hard-bans.md", hardBansMd)} variant="primary" size="sm">Save</Button></div>
  </ExpandCard>

  <ExpandCard title="Escalation Matrix">
    <textarea class="mono-editor" rows="10" bind:value={escalationMd}></textarea>
    <div class="card-actions"><Button onclick={() => saveFile("escalation-matrix.md", escalationMd)} variant="primary" size="sm">Save</Button></div>
  </ExpandCard>

  <ExpandCard title="Chat Flow">
    <div class="flow-grid">
    ...
    <div class="card-actions"><Button onclick={saveChatFlow} variant="primary" size="sm">Save</Button></div>
  </ExpandCard>

  <ExpandCard title="General Settings">
    <div class="flow-grid">
      <div class="form-group">
        <label>Bot Name
