      ...
      <tbody>
        {#each items as item}
          <tr>
            ...
            <td class="actions">
              <Button onclick={() => openEditEntry(item)} variant="ghost" size="sm">Duzenle</Button>
              <Button onclick={() => deleteEntry(item.id || item._id)} variant="ghost" size="sm">Sil</Button>
            </td>
          </tr>
        {:else}
...
<!-- Edit Entry Modal -->
<Modal bind:open={editOpen} title="Kayit Duzenle">
  <div class="form-group">
    <label>Soru
      <input class="input" bind:value={editQuestion} placeholder="Soru..." />
    </label>
  </div>
  <div class="form-group">
    <label>Cevap
      <textarea class="textarea" bind:value={editAnswer} rows="4" placeholder="Cevap..."></textarea>
    </label>
  </div>
  <div class="modal-actions">
    <Button onclick={() => (editOpen = false)} variant="secondary">Iptal</Button>
    <Button onclick={updateEntry} variant="primary">Kaydet</Button>
  </div>
</Modal>
